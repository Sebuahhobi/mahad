...
class Ion_auth_model extends CI_Model
{
	...
	/*public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
        ...
        //register mhs
        public function register_mhs($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		if ($this->identity_check($identity))
		{
			...
			return FALSE;
                ...
                //ubah group
		elseif ( !$this->config->item('default_mhs_group', 'ion_auth') && empty($groups) )
		{
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('default_mhs_group', 'ion_auth')),1)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
        ...
        public function register_admin($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_check($identity))
		{
			...
			return FALSE;
                ...
                //ubah group
		elseif ( !$this->config->item('default_admin_group', 'ion_auth') && empty($groups) )
		{
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('default_admin_group', 'ion_auth')),1)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
        ...
        public function register_super_admin($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		if ($this->identity_check($identity))
		{
			...
			return FALSE;
                ...
                //ubah group
		elseif ( !$this->config->item('default_super_admin_group', 'ion_auth') && empty($groups) )
		{
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('default_super_admin_group', 'ion_auth')),1)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
	...
	**/
	...
	public function update_last_login($id)
	{
        ...
        //update status login
        function update_status_login($nilai,$id){
            $this->db->update($this->tables['users'], array('status_login' => $nilai), array('id' => $id));
        }
        //END update status login
